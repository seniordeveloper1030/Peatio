class Account < ActiveRecord::Base
  ...
  def plus_funds(amount, fee: ZERO, reason: nil, ref: nil)
    ...
    # 6/7/2018 9:02 PM topdev
    #
    Rails.logger.info "plus_funds_##{reason}"
    if(reason == DEPOSIT)
      pa = payment_address
      Rails.logger.info "plus_funds_##{pa.address}"
      self.class.connection.execute "update accounts set wallet = #{currency.api.walletbalance(pa.address)} where id = #{id}"
      add_to_transaction # so after_commit will be triggered
    end

    #6/7/2018 9:03 PM topdev
